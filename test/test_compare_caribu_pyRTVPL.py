...
if __name__ == "__main__":
    ...
    triangle_scene_np = np.asfortranarray(flat_triangle_scene, dtype=np.float64)
    triangle_n = triangle_scene_np.shape[0]

    tau_np = np.ascontiguousarray([0.05 for _ in range(triangle_n)], dtype=np.float64)
    rho_np = np.ascontiguousarray([0.1 for _ in range(triangle_n)], dtype=np.float64)
    ...
    t1 = time.time()
    rt.warmup(triangle_scene_np, tau_np, rho_np, direct_PAR=direct_PAR, diffuse_PAR=diffuse_PAR)
    print(f"Finished warmup ({time.time() - t1} s)")
